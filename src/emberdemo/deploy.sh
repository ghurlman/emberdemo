...
tar cf - --exclude=node_modules --exclude=bower_components --exclude=wwwroot --exclude=tmp --exclude=.git . | (cd $DEPLOYMENT_TEMP && tar xvf - )
...
if [[ "$IN_PLACE_DEPLOYMENT" -ne "1" ]]; then
  "$KUDU_SYNC_CMD" -v 50 -f "$DEPLOYMENT_TEMP/wwwroot" -t "$DEPLOYMENT_TARGET" -n "$NEXT_MANIFEST_PATH" -p "$PREVIOUS_MANIFEST_PATH" -i ".git;.hg;.deployment;deploy.sh"
